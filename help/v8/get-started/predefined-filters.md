...
>title="Salva filtro"
>abstract="Seleziona un filtro predefinito esistente. I filtri preimpostati sono filtri personalizzati creati e salvati per utilizzi futuri. Fungono da scelte rapide durante le operazioni di filtro mediante query modeler. Ad esempio, possono essere utilizzati per filtrare un elenco di dati o creare il pubblico di una consegna."
...
>title="Salva filtro"
>abstract="Questa schermata ti consente di salvare la regola come filtro predefinito. Puoi creare un nuovo filtro predefinito o sovrascrivere un filtro esistente. Per salvare il filtro come preferito, attiva l’opzione **[!UICONTROL Filtro preferito]**. È inoltre possibile abilitare l’opzione **[!UICONTROL Filtro condiviso]** per rendere il filtro accessibile ad altri utenti."
...
1. Seleziona **Crea un nuovo filtro** e immetti il nome e la descrizione del filtro.
   ...
   Se necessario, puoi salvare il filtro come preferito. Per ulteriori informazioni, consulta [questa sezione](#fav-filter). È inoltre possibile abilitare l’opzione **[!UICONTROL Filtro condiviso]** per rendere il filtro accessibile ad altri utenti. Per ulteriori informazioni, consulta [questa sezione](#share-filter).
...
Il filtro viene aggiunto all’elenco dei filtri preimpostati. Se necessario, puoi salvare il filtro come preferito. Per ulteriori informazioni, consulta [questa sezione](#fav-filter). Puoi anche condividere il filtro con altri utenti abilitando l’opzione **[!UICONTROL Filtro condiviso]**. Per ulteriori informazioni, consulta [questa sezione](#share-filter).
...
![Sezione Filtri preferiti](assets/predefined-filters-favorite.png){zoomable="yes"}
...
![Opzione filtro condiviso](assets/predefined-filters-shared.png){zoomable="yes"}

I filtri condivisi vengono visualizzati nell’elenco dei filtri preimpostati per tutti gli utenti, consentendo loro di utilizzarli nei propri flussi di lavoro e nelle proprie query.
...
1. Passa al filtro preimpostato da usare per creare il pubblico, selezionalo e conferma.
   ...
   Puoi anche modificare o eliminare un filtro preimpostato esistente utilizzando il pulsante ![](../content/assets/do-not-localize/Smock_More_18_N.svg).
